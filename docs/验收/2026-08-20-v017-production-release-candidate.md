...
1. 从公开 `v0.1.16` 使用同证书普通覆盖安装，不卸载且不使用降级参数。
...
本文件记录的是本地正式签名 Release 候选。候选尚未人工上传到固定公网地址，也未从公网重新下载回验；`download.html` 继续推荐已公开回验的 `v0.1.16`。完成上传与公网哈希核对前，不得把 `v0.1.17` 写成已公开分发。
