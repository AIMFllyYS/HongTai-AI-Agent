...
> 状态：救援分支候选已通过主机和 API 35 模拟器门禁，正式签名 APK 后续已公开分发；物理真机与公开 Debug→Release 迁移仍未验证
>
> 日期：2026-08-13
>
...
源码候选已经从公开推荐测试版 `0.1.4`/11 单调递增到 `0.1.5`/12。本记录完成后，正式签名 APK 已上传至 `https://husteread.com/storage/public/HongTai-AI-Agent-release-v0.1.5.apk`，公开文件为 25,943,725 字节，SHA-256 为 `48D65860532BF1641222173BA42FFE479EA3180B3B75A146357EB44C25D1DE6D`；`download.html` 已将其设为当前推荐版本。该后续分发事实不补写本记录当时缺失的物理真机证据。
...
本记录形成时，可以称为“`v0.1.5/code12` 正式签名 Release 候选已构建，并通过主机与 API 35 模拟器门禁”。其后该 APK 已公开分发，因此现在可以确认“v0.1.5 已公开发布”，但仍不能称为“物理真机通过”或“现有 v0.1.4 用户可直接覆盖升级”。未完成项有两个：一是没有连接物理 Android 设备，未覆盖 OEM 相机/相册、真实媒体、云端 TTS、长任务最小化/进程回收等门禁；二是公开 v0.1.4 使用 Debug 证书，不能覆盖升级到正式证书 APK，仍需要数据导出/迁移与一次性卸载重装说明。

后续发布动作没有改变上述设备证据边界。不得用“公网可下载”倒推出物理设备或 OEM 行为已经验证；备份引用、工作树和历史产物仍不得在无明确授权时删除。
