...
| 3 生命周期终态 | [#43](https://github.com/AIMFllyYS/HongTai-AI-Agent/issues/43) | #44 #45 #46 #47 #48 #49 | core / Android I/O / node-runtime | `packages/core`、`android/app`、`packages/node-runtime`、`apps/web/src/features/tasks` |
...
1. **禁止**使用任何基于 worktree 的并行执行机制（包括 best-of-N 类的隔离运行器）来并行改代码。
2. **禁止**同时运行两个会写文件的子代理。它们共享同一个根工作区，会互相覆盖，且 `git status` 无法区分是谁改的。
3. 子代理只用于**并行只读**工作：定位代码、复核 Issue 描述的事实是否仍成立、分析跨层影响面、以及改完之后的独立复查。

实现与提交必须串行，一次一个 sub-issue。这不是效率问题，是正确性问题。

推荐的子代理用法：

- **开工前**：每个 sub-issue 派一个只读代理复核「当前行为」段落是否仍然成立。审查报告基于 `main` 分支 HEAD `47b7b2d`，如果执行时已经有新提交，部分描述可能已经过时。
- **实现中**：遇到跨层影响面不清楚时，派只读代理去把调用链摸清楚，主线程继续等结论。
- **收尾时**：派一个代理对本批 diff 做防回归复查，重点看是否引入了跨层越界、并发覆盖、生命周期丢失或真实性问题。
...
把 `{N}` 换成批次号，`{PARENT}` 换成 parent issue 编号，直接贴进新会话。
...
开工前依次读完这四份：
1. AGENTS.md
2. docs/任务/2026-08-16-修复批次执行总纲.md（本批的归属目录、验证要求、子代理纪律、PR 形式都在里面）
3. GitHub Issue #{PARENT} 及其全部 sub-issue（用 gh issue view 读正文，注意 PowerShell 下不要用带管道符的 --jq 表达式）
4. 各 sub-issue 正文里点名的源文件

执行方式：

第一步，复核。为每个 sub-issue 派一个只读子代理，核对 Issue 正文「当前行为」段落描述的事实在当前 main 上是否仍然成立。Issue 基于 2026-08-16 的审查快照，可能已经过时。发现已失效的，报告给我，不要默默跳过也不要默默照做。

第二步，实现。严格串行，一次一个 sub-issue，一个 commit。绝对不要开 worktree，也不要同时跑两个写文件的子代理——本仓库只在根目录工作，并行会互相覆盖。每完成一个 sub-issue 立刻跑它自己的定向测试。

第三步，验证。按总纲的「验证映射」跑本批要求的全部命令。Android 相关的改动，没有真机证据就明确写「未经真机验证」，不要含糊过去。

第四步，交付。从最新 main 切分支 fix/batch-{N}-<slug>，push，开 PR。PR 正文写变更摘要、每项的测试命令与结果摘要、风险与回滚思路，末尾列出本批全部 Closes #NNN。

约束：
- 不改 Issue 范围之外的代码。发现新问题就登记新 Issue，不要顺手扩写。
- 不为了让测试变绿而弱化断言或删用例。
- 不推进 versionName / versionCode。
- 不推送到 main，只推工作分支。
```
