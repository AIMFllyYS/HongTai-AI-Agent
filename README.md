...
> 当前能力、发布边界和修复优先级见[当前能力与发布状态](docs/当前能力与发布状态.md)，版本与构建规则见[版本与发布流程](docs/版本与发布流程.md)。Android 源码候选当前为 `v0.1.12`（`versionCode=20`），仓库下载页当前推荐 `v0.1.11`（`versionCode=19`）。物理设备、真实模型调用、人工上传和公网文件回验完成前，v0.1.12 只是 Release 候选，不是正式发布版本。
