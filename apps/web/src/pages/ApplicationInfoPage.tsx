...
const latestUpdates = [
  "制作支持一键全自动：智能成片、爆款复刻、数字人三种方式都只需要上传素材，其余流程自动完成，进度实时可见。",
  "分镜脚本更好用了：AI 会自动配置字幕强调词并在字幕里正确应用，脚本生成过程流式呈现，不用干等。",
  "数字人模式更简单：只需上传一个预处理好的数字人视频，AI 自动裁剪、拼接画面对齐配音时长。",
  "合成更稳了：修复了多个合成失败的问题，时长校验从硬性卡断改为提示确认，加载过程有骨架屏不再白屏。",
  "问题不再被藏起来：一键流程中出错会明确显示原因和软违规提示，不会假装成功或一直转圈。",
] as const;
