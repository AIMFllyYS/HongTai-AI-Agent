...
import { matchRoute, pathForRoute, splitLocationHref } from "../apps/web/src/router";
...
test("加号三项只指向已接入的真实入口", () => {
  ...
  for (const action of composeActions) {
    const href = pathForComposeAction(action.id);
    assert.equal(matchRoute(href).key, matchRoute(splitLocationHref(href).pathname).key);
    assert.notEqual(matchRoute(href).key, "not-found", href);
  }
});
...
test("compose sheet, home paste intent, and create swipe lock stay wired", () => {
  ...
  assert.match(create, /\[runtime, searchEpoch\]/);
  assert.match(browserRoute, /splitLocationHref/);
  assert.match(browserRoute, /setPathname\(nextPathname\)/);
  assert.doesNotMatch(browserRoute, /setPathname\(path\)/);
