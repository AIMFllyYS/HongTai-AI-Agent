...
test("the v0.1.12/code 20 source candidate advances beyond the published v0.1.11/code 19", () => {
  ...
  assert.equal(candidateCode, 20);
  assert.equal(candidateName, "0.1.12");
  assert.equal(publishedName, "0.1.11");
...
test("the repository maintains a changelog and a patch-only default version policy", () => {
  ...
  assert.match(changelog, /Android 源码候选.*`0\.1\.12`.*`versionCode=20`/u);
