...
test("the v0.1.11/code 19 source candidate advances beyond the published v0.1.8/code 16", () => {
  ...
  assert.equal(candidateCode, 19);
  assert.equal(candidateName, "0.1.11");
...
test("the repository maintains a changelog and a patch-only default version policy", () => {
  ...
  assert.match(changelog, /Android 源码候选.*`0\.1\.11`.*`versionCode=19`/u);
