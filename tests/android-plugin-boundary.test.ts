...
test("release candidate v0.1.29 keeps the HongTai brand source and ships an adaptive launcher icon", () => {
  const appBuild = read("android/app/build.gradle.kts");
  const manifest = read("android/app/src/main/AndroidManifest.xml");
  ...
  assert.match(appBuild, /versionCode\s*=\s*37\b/);
  assert.match(appBuild, /versionName\s*=\s*"0\.1\.29"/);
