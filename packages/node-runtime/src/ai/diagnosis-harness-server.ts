...
document.querySelector('#upload').onclick=async()=>{try{const file=document.querySelector('#image').files[0];if(!file)throw new Error('请选择图片');status.textContent='分析中…';const imageDataUrl=await new Promise((ok,bad)=>{const r=new FileReader();r.onload=()=>ok(r.result);r.onerror=bad;r.readAsDataURL(file)});const res=await fetch('/api/sessions',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({mode:document.querySelector('#mode').value,imageDataUrl})});const data=await res.json();if(!res.ok)throw new Error(data.error);sessionId=data.sessionId;document.querySelector('#report').innerHTML='<h2>'+data.report.summary.headline+'</h2><div class="message">'+data.report.summary.narrative+'</div><ul>'+data.report.summary.keyPoints.map(x=>'<li>'+x+'</li>').join('')+'</ul>';status.textContent='完成：'+sessionId}catch(e){status.className='error';status.textContent=e.message}};
document.querySelector('#send').onclick=async()=>{try{if(!sessionId)throw new Error('请先上传图片');const question=document.querySelector('#question').value.trim();const res=await fetch('/api/sessions/'+sessionId+'/messages',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({question})});const data=await res.json();if(!res.ok)throw new Error(data.error);document.querySelector('#messages').insertAdjacentHTML('beforeend','<div class="message">你：'+question+'</div><div class="message">AI：'+data.message.content+'</div>');document.querySelector('#question').value=''}catch(e){status.className='error';status.textContent=e.message}};
</script></body></html>`;
