...
export function contentAnalysisPrompt(input: ContentAnalysisInput): string {
  return `${RULES}\n任务来源：${JSON.stringify({ taskId: input.taskId, platform: input.platform, contentType: input.contentType, sourceKind: input.sourceKind, title: input.title, author: input.author })}\n证据单元：${JSON.stringify(input.evidenceUnits)}`;
}
